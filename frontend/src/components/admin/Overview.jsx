  ...
  return (
    <div className="space-y-6 fade-up" data-testid="admin-analytics">
      <div>
        ...
        <p className="text-sm text-slate-500">வணிக சுருக்கம் — revenue, profit & bills at a glance</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat icon={<IndianRupee size={18} className="text-emerald-700" />} label="Total Revenue" value={fmt(data.totalRevenue)} sub={`${data.totalBills} bills`} tone="bg-emerald-50" />
        <Stat icon={<TrendingUp size={18} className="text-amber-600" />} label="Net Profit" value={fmt(data.netProfit)} sub="From delivered orders" tone="bg-amber-50" />
        <Stat icon={<Wallet size={18} className="text-blue-600" />} label="Collected" value={fmt(data.collected)} sub={`${fmt(data.pendingAmount)} pending`} tone="bg-blue-50" />
        <Stat icon={<ReceiptText size={18} className="text-purple-600" />} label="Avg Bill Value" value={fmt(data.avgOrderValue)} sub="Per order" tone="bg-purple-50" />
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5" data-testid="order-status-summary">
          ...
          {["Pending", "Shipped", "Delivered"].map((s) => (
            <div key={s} className="flex items-center justify-between py-2.5 border-b border-dashed border-slate-100 last:border-0">
              ...
              <span className="text-lg font-extrabold text-slate-800" data-testid={`status-count-${s.toLowerCase()}`}>{data.statusCounts[s]}</span>
            </div>
          ))}
        </div>
              ...
              {data.recentOrders.map((o) => (
                <div key={o.id} className="flex items-center justify-between border border-slate-100 rounded-xl px-3 py-2">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">{o.customerName || o.customerMobile}</p>
                  </div>
                  <span className="text-sm font-extrabold text-emerald-700">{fmt(o.total)}</span>
                </div>
              ))}
            </div>
          )}
